...
const ContractManagement = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const validResidents = Array.isArray(rList) ? rList : [];
    ...
    const vacantApartments = apartments.filter(a => {
        const st = a.status ?? a.Status;
        return st === 1 || String(st).toLowerCase() === 'vacant';
    });
        ...
        <div className="container-fluid p-0">
            ...
            <div className="modal fade" id="createContractModal" tabIndex="-1">
                <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                    ...
                    {/* THAY ĐỔI Ở ĐÂY: Dùng form làm class modal-content luôn để giữ đúng cấu trúc Flexbox */}
                    <form className="modal-content" onSubmit={handleCreateContract} id="contractForm">
                                    ...
                                    <div className="row mb-3">
                                        <div className="col-md-6">
                                            ...
                                            <Select
                                                ...
                                                value={activeResidents.find(r => Number(r.accountId ?? r.AccountId) === Number(formData.residentAccountId)) ? { 
                                                    label: `${activeResidents.find(r => Number(r.accountId ?? r.AccountId) === Number(formData.residentAccountId)).fullName || activeResidents.find(r => Number(r.accountId ?? r.AccountId) === Number(formData.residentAccountId)).FullName || 'N/A'} - CCCD: ${activeResidents.find(r => Number(r.accountId ?? r.AccountId) === Number(formData.residentAccountId)).identityCard || activeResidents.find(r => Number(r.accountId ?? r.AccountId) === Number(formData.residentAccountId)).IdentityCard || 'N/A'}`, 
                                        ...
                                        <div className="col-md-6">
                                            ...
                                            <Select
                                                ...
                                                value={vacantApartments.find(a => Number(a.apartmentId ?? a.ApartmentId) === Number(formData.apartmentId)) ? { 
                                                    label: `${vacantApartments.find(a => Number(a.apartmentId ?? a.ApartmentId) === Number(formData.apartmentId)).apartmentCode || vacantApartments.find(a => Number(a.apartmentId ?? a.ApartmentId) === Number(formData.apartmentId)).ApartmentCode} - Tòa B`, 
                                            ...
                                            <div className="row mb-2 align-items-end" key={index}>
                                                <div className="col-md-6">
                                                    ...
                                                    >
                                                        ...
                                                        {activeResidents.filter(r => Number(r.accountId || r.AccountId) !== Number(formData.residentAccountId)).map(r => (
